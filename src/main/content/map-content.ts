...
export class MapContentAPI extends PrDownloaderAPI<MapData> {
    ...
    public override async init() {
        ...
        const maps = await this.cacheDb.selectFrom("map").selectAll().execute();
    ...
    public async queueMapsToCache() {
        ...
        const cachedMapFiles = await this.cacheDb.selectFrom("map").select(["fileName"]).execute();
        const cachedMapFileNames = cachedMapFiles.map((file) => file.fileName);
        const erroredMapFiles = await this.cacheDb.selectFrom("mapError").select(["fileName"]).execute();
    ...
    protected async uncacheMap(fileName: string) {
        ...
        await this.cacheDb.deleteFrom("map").where("fileName", "=", fileName).execute();
    ...
    protected async cacheMap(mapFileName: string) {
        try {
            ...
            const existingCachedMap = await this.cacheDb.selectFrom("map").select("mapId").where("fileName", "=", fileName).executeTakeFirst();
            ...
            const mapData = await this.cacheDb
                .insertInto("map")
                .values({
                    ...parsedMap,
