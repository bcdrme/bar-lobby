...
export class GameContentAPI extends PrDownloaderAPI<GameVersion | CustomGameVersion> {
    ...
    public override async init() {
        ...
        const gameVersions = await this.cacheDb.selectFrom("gameVersion").selectAll().execute();
    ...
    protected async uncacheVersion(version: GameVersion) {
        await this.cacheDb.deleteFrom("gameVersion").where("id", "=", version.id).execute();
    }
    ...
    protected async addGame(id: string, sort = true) {
        ...
        const gameVersion = await this.cacheDb.insertInto("gameVersion").values({ id, md5, ais, lastLaunched: new Date() }).returningAll().executeTakeFirstOrThrow();
    ...
    protected async cleanupOldVersions() {
        ...
        const versionsToRemove = await this.cacheDb.selectFrom("gameVersion").where("lastLaunched", "<", oldestDate).select("id").execute();
