...
import { extract7z, removeFromArray } from "$/jaz-ts-utils";
...
import { CacheDb, getCacheDb } from "@main/cache/cache-db";
...
export class EngineContentAPI extends AbstractContentAPI<EngineVersion> {
    ...
    public override async init() {
        this.cacheDb = await getCacheDb();
        await fs.promises.mkdir(this.engineDirs, { recursive: true });
        const engineVersions = await this.cacheDb.selectFrom("engineVersion").selectAll().execute();
    ...
    protected async uncacheVersion(id: string) {
        await this.cacheDb.deleteFrom("engineVersion").where("id", "=", id).execute();
    }
    ...
    protected async addEngine(id: string, sort = true) {
        ...
        const engineVersion = await this.cacheDb.insertInto("engineVersion").values({ id, ais, lastLaunched: new Date() }).returningAll().executeTakeFirstOrThrow();
    ...
    protected async cleanupOldVersions() {
        ...
        const versionsToRemove = await this.cacheDb.selectFrom("engineVersion").where("lastLaunched", "<", oldestDate).select("id").execute();
