...
export class SpadsBattle extends AbstractBattle<SpadsBattleOptions> {
    ...
    protected updateSync() {
        ...
        if (engineContentAPI.isVersionInstalled(this.battleOptions.engineVersion)) {
            sync.engine = 1;
        } else {
            const dl = engineContentAPI.currentDownloads.find((dl) => dl.name === this.battleOptions.engineVersion);
            if (dl) {
                sync.engine = dl.currentBytes / dl.totalBytes;
            }
        }

        if (gameContentAPI.isVersionInstalled(this.battleOptions.gameVersion)) {
            sync.game = 1;
        } else {
            const dl = gameContentAPI.currentDownloads.find((dl) => dl.name === this.battleOptions.gameVersion);
            if (dl) {
                sync.game = dl.currentBytes / dl.totalBytes;
            }
        }

        if (mapContentAPI.isVersionInstalled(this.battleOptions.map)) {
            sync.map = 1;
        } else {
            const dl = mapContentAPI.currentDownloads.find((dl) => dl.name === this.battleOptions.map);
            if (dl) {
