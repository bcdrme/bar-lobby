...
class CacheDbAPI extends Kysely<CacheDatabase> {
    protected migrator: Migrator;
    // protected serializePlugin: SerializePlugin;
    
    constructor() {
        // const serializePlugin = new SerializePlugin();
        super({
            dialect: new SqliteDialect({
                database: new Database(path.join(getInfo().contentPath, "cache.db")),
            }),
            // plugins: [serializePlugin], // is this needed?
        });
...
let instance: CacheDbAPI | null = null;
export type CacheDb = CacheDbAPI;
export async function getCacheDb() {
    if (!instance) {
        instance = new CacheDbAPI();
        await instance.init();
    }
    return instance;
}
